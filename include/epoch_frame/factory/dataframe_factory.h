...
namespace arrow
{
    template <> struct CTypeTraits<epoch_frame::DateTime>
    {
...
namespace epoch_frame
{
    DataFrame make_empty_dataframe(arrow::DataTypePtr const& indexType,
                                   arrow::SchemaPtr const&   schema = arrow::schema({}));
                             ...
                             std::vector<std::string> const& columnNames)
    {
        ...
        for (auto const& [name, column] : std::views::zip(columnNames, data))
        {
            ...
            {
                columnBuilder =
                    std::make_unique<typename arrow::CTypeTraits<ColumnT>::BuilderType>();
            }
