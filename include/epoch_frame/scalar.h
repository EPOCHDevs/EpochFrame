...
namespace epoch_frame
{
    ...
    inline arrow::ScalarPtr MakeScalar(const char* value)
    {
    ...
    class Scalar
    {
      public:
        ...
        template <typename T>
            requires std::is_scalar_v<T>
        explicit Scalar(T other) : Scalar(MakeScalar(std::move(other)))
        {
        }
        ...
        [[nodiscard]] arrow::TimestampScalar
        timestamp(std::string const& format = "%Y-%m-%d %H:%M:%S",
                  const std::string& tz     = "") const;

        DateTime to_datetime(std::string const& format = "%Y-%m-%d %H:%M:%S",
                             const std::string& tz     = "") const;
      ...
      private:
        ...
        template <typename T> T get_or_throw_value() const
        {
            try
            {
            ...
            catch (std::exception const& e)
            {
                throw std::runtime_error(
                    std::format("Failed to convert scalar to {}: {}", typeid(T).name(), e.what()));
            }
        }
    };
    ...
    template <typename T> using ScalarMapping = std::unordered_map<Scalar, T, ScalarHash>;
