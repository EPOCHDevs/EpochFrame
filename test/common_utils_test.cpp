...
TEST_CASE("arrow_utils::call_unary_compute - valid kernel (sum on double array)",
          "[arrow][compute]")
{
    auto arr = make_array<double>(
        std::vector<double>{1., 2., 3., 4., std::numeric_limits<double>::quiet_NaN()});
...
TEST_CASE("arrow_utils::call_unary_compute - unknown kernel", "[arrow][compute]")
{
    ...
    REQUIRE_THROWS_WITH(call_unary_compute(arr, "unknown_kernel"),
...
TEST_CASE("arrow_utils::call_unary_compute_as<T>", "[arrow][compute]")
{
    ...
    REQUIRE_THROWS_WITH(call_unary_compute_scalar_as<arrow::DoubleScalar>(arr, "sum", &agg_opts),
...
TEST_CASE("arrow_utils::shift handles positive, negative, overshift, zero, empty", "[arrow][shift]")
{
    using namespace epoch_frame::factory::array;
    using namespace epoch_frame::arrow_utils;

    // Base array: [1,2,3,4]
    auto arr = make_array<int64_t>(std::vector<int64_t>{1, 2, 3, 4});
    REQUIRE(arr->length() == 4);

    // Positive shift by 2 => [null,null,1,2]
    {
        auto result = shift(epoch_frame::TableOrArray{arr}, 2).chunked_array();
        CHECK(result->length() == 4);

        auto expected = epoch_frame::factory::array::join_chunked_arrays(
            make_null_array(2, arr->type()), arr->Slice(0, 2),
            /*join_right=*/true);
        CHECK(expected->Equals(result));
    }

    // Negative shift by -2 => [3,4,null,null]
    {
        auto result = shift(epoch_frame::TableOrArray{arr}, -2).chunked_array();
        CHECK(result->length() == 4);

        auto expected = epoch_frame::factory::array::join_chunked_arrays(
            make_null_array(2, arr->type()), arr->Slice(2, 2),
            /*join_right=*/false);
        CHECK(expected->Equals(result));
    }

    // Overshift by 10 => all nulls of length 4
    {
        auto result = shift(epoch_frame::TableOrArray{arr}, 10).chunked_array();
        CHECK(result->length() == 4);

        auto expected_nulls = make_null_array(4, arr->type());
        auto expected       = make_array(expected_nulls);
        CHECK(expected->Equals(result));
    }

    // Zero shift => identity
    {
        auto result = shift(epoch_frame::TableOrArray{arr}, 0).chunked_array();
        CHECK(result->Equals(arr));
    }

    // Empty input => empty output
    {
        auto empty = make_array<int64_t>(std::vector<int64_t>{});
        REQUIRE(empty->length() == 0);
        auto result = shift(epoch_frame::TableOrArray{empty}, 3).chunked_array();
        CHECK(result->length() == 0);
        CHECK(result->type()->Equals(empty->type()));
    }
}
...
TEST_CASE("arrow_utils::call_unary_agg_compute_as - invalid type cast", "[arrow][compute]")
{
    ...
    REQUIRE_THROWS_WITH(call_unary_compute_scalar_as<arrow::Int64Scalar>(arr, "all"),
