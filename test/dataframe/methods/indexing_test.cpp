...
TEST_CASE("Indexing Test")
{
    ...
    auto default_frame = make_dataframe(from_range(5), std::vector{a, b, c}, {"A", "B", "C"});

    auto empty_frame = make_dataframe(from_range(0, 0), std::vector<std::vector<int>>{{}, {}, {}},
                                      {"A", "B", "C"});
    ...
    SECTION("iloc")
    {
        ...
        params.push_back(TestCase{.title  = "Positive IIndex",
                                  .frame  = default_frame,
                                  .input  = std::make_tuple(2, "B"),
                                  .output = Scalar{30}});

        params.push_back(TestCase{.title  = "Negative IIndex",
                                  .frame  = default_frame,
        ...
        params.push_back(TestCase{
            ...
            .output = make_dataframe(
                from_range(1, 4), std::vector{std::vector{2, 3, 4}, {20, 30, 40}, {200, 300, 400}},
        ...
        params.push_back(TestCase{
            ...
            .output = make_dataframe(
                from_range(0, 3), std::vector{std::vector{1, 2, 3}, {10, 20, 30}, {100, 200, 300}},
                default_frame.column_names())});
        ...
        params.push_back(TestCase{
            ...
            .output = make_dataframe(
                from_range(2, 5), std::vector{std::vector{3, 4, 5}, {30, 40, 50}, {300, 400, 500}},
                default_frame.column_names())});
        ...
        params.push_back(TestCase{
            ...
            .frame = default_frame,
            .input =
                UnResolvedIntegerSliceBound{-3, static_cast<int64_t>(default_frame.num_rows())},
            .output = make_dataframe(from_range(2, 5), // rows 2 to 4 (i.e. 2,3,4)
                                     std::vector{std::vector{3, 4, 5}, std::vector{30, 40, 50},
        ...
        params.push_back(TestCase{
            ...
            .output = make_dataframe(from_range(1, 4), // rows 1,2,3
                                     std::vector{std::vector{2, 3, 4}, std::vector{20, 30, 40},
                                                 std::vector{200, 300, 400}},
                                     default_frame.column_names())});

        // Variation 6: Slice with start >= end should return an empty frame.
        params.push_back(
            TestCase{.title  = "Empty slice when start >= end",
                     .frame  = default_frame,
                     .input  = UnResolvedIntegerSliceBound{4, 2},
                     .output = make_dataframe(from_range(0, 0),                          // no rows
                                              std::vector<std::vector<int>>{{}, {}, {}}, // no data
                                              default_frame.column_names())});

        // Variation 7: Out-of-bound slice [3,10) → rows 3 and 4 only.
        params.push_back(
            TestCase{.title  = "Out-of-bound slice [3,10)",
                     .frame  = default_frame,
                     .input  = UnResolvedIntegerSliceBound{3, 10},
                     .output = make_dataframe(
                         from_range(3, 5), // rows 3 and 4
                         std::vector{std::vector{4, 5}, std::vector{40, 50}, std::vector{400, 500}},
                         default_frame.column_names())});

        params.push_back(TestCase{.title  = "Slice on empty table",
                                  .frame  = empty_frame,
        ...
        params.push_back(TestCase{.title  = "Single-row slice [3:4]",
                                  .frame  = default_frame,
                                  .input  = UnResolvedIntegerSliceBound{3, 4},
                                  .output = make_dataframe(from_range(3, 4),
        ...
        params.push_back(
            TestCase{.title  = "Negative start and stop slice [-4:-2]",
                     .frame  = default_frame,
                     .input  = UnResolvedIntegerSliceBound{-4, -2},
                     .output = make_dataframe(from_range(1, 3), // rows 1 and 2
    ...
    SECTION("operator []")
    {
        ...
        params.push_back(
            {.title  = "Multiple columns access",
             ...
             .output = make_dataframe(
                 default_frame.index(),
                 std::vector{std::vector{1, 2, 3, 4, 5}, std::vector{100, 200, 300, 400, 500}},
                 StringVector{"A", "C"})});

        // Empty column selection
        params.push_back({.title  = "Empty column selection",
                          .frame  = default_frame,
                          .input  = StringVector{},
                          .output = DataFrame{}});

        // Non-existent column access (should throw)
        params.push_back({.title  = "Non-existent column access",
                          .frame  = default_frame,
        ...
        params.push_back(
            {.title = "Filter columns using callable",
             .frame = default_frame,
             .input = StringVectorCallable(
                 [](const StringVector& cols) -> StringVector
                 {
                     StringVector result;
             ...
             .output = make_dataframe(
                 default_frame.index(),
                 std::vector{std::vector{10, 20, 30, 40, 50}, std::vector{100, 200, 300, 400, 500}},
                 StringVector{"B", "C"})});
        ...
        for (auto const& [title, frame, input, output] : params)
        {
            ...
            {
                std::visit(
                    [&]<typename T1, typename T2>(const T1& arg, const T2& expected)
                    {
    ...
    SECTION("loc")
    {
        ...
        SECTION("loc - Selecting Multiple Rows")
        {
            SECTION("Selecting multiple rows [1, 3, 4]")
            {
                ...
                DataFrame expected =
                    make_dataframe(make_range({1, 3, 4}, MonotonicDirection::Increasing),
                                   std::vector{std::vector{2, 4, 5}, std::vector{20, 40, 50},
        ...
        SECTION("loc - Boolean Filtering")
        {
            SECTION("Filtering rows where column 'A' > 2")
            {
                ...
                DataFrame expected =
                    make_dataframe(make_range({2, 3, 4}, MonotonicDirection::Increasing),
                                   std::vector{std::vector{3, 4, 5}, std::vector{30, 40, 50},
                                               std::vector{300, 400, 500}},
                                   default_frame.column_names());
            ...
            {
                Series filter = make_series(default_frame.index(),
                                            std::vector<bool>{true, false, true, false, true});

                DataFrame expected =
                    make_dataframe(make_range({0, 2, 4}, MonotonicDirection::Increasing),
                                   std::vector{std::vector{1, 3, 5}, std::vector{10, 30, 50},
        ...
        SECTION("loc - Selecting with a Callable Function")
        {
            SECTION("Using callable to filter rows where column 'B' > 25")
            {
                ...
                DataFrame expected = make_dataframe(
                    make_range(
                        {2, 3, 4},
                        MonotonicDirection::Increasing), // Rows 3 and 4 (since B = 40, 50 > 25)
                    std::vector{std::vector{3, 4, 5}, std::vector{30, 40, 50},
                                std::vector{300, 400, 500}},
                    default_frame.column_names());
    ...
    SECTION("reindex")
    {
        ...
        {
            auto partial_index =
                make_range(std::vector<uint64_t>{2, 4, 6}, MonotonicDirection::Increasing);
        ...
        {
            auto incompatible_index =
                make_object_index(std::vector<std::string>{"row1", "row2", "row3"});
        ...
        // {
        //     REQUIRE_THROWS(
        //         make_range(std::vector<uint64_t>{1, 2, 2, 3, 4},
        ...
        {
            auto sparse_index =
                make_range(std::vector<uint64_t>{0, 2, 5, 8}, MonotonicDirection::Increasing);
    ...
    SECTION("where")
    {
        SECTION("where - Basic filtering with scalar")
        {
            ...
            auto                  expected =
                make_dataframe(default_frame.index(),
                               std::vector{std::vector<int64_t>{999, 999, 3, 4, 5},
        ...
        SECTION("where - Condition as Series, Other as Scalar")
        {
            ...
            DataFrame             result    = default_frame.where(condition, other);
            auto                  expected  = make_dataframe(default_frame.index(),
                                                             std::vector{std::vector<int64_t>{0, 0, 3, 4, 5},
        ...
        SECTION("where - Condition as Callable, Other as DataFrame")
        {
            ...
            WhereOtherVariant other =
                make_dataframe(default_frame.index(),
                               std::vector{std::vector<int64_t>{10, 20, 30, 40, 50},
                                           std::vector<int64_t>{10, 20, 30, 40, 50},
                                           std::vector<int64_t>{10, 20, 30, 40, 50}},
                               default_frame.column_names());
            DataFrame result   = default_frame.where(condition, other);
            auto      expected = make_dataframe(default_frame.index(),
                                                std::vector{std::vector<int64_t>{1, 2, 3, 40, 50},
                                                       std::vector<int64_t>{10, 20, 30, 40, 50},
        ...
        SECTION("where - Condition as DataFrame, Other as Scalar")
        {
            ...
            auto expected = make_dataframe(
                default_frame.index(),
                std::vector{
                    std::vector<int64_t>{0, 0, 3, 4,
                                         0}, // for column "A": {1,2,3,4,5} → {0,0,3,4,0}
                    std::vector<int64_t>{10, 0, 30, 40,
                                         0}, // for column "B": {10,20,30,40,50} → {10,0,30,40,0}
                    std::vector<int64_t>{0, 200, 0, 400, 500}
                    // for column "C": {100,200,300,400,500} → {0,200,0,400,500}
                },
                default_frame.column_names());
            INFO("Result:\n" << result << "\nExpected:\n" << expected);
            REQUIRE(result.equals(expected));
        ...
        SECTION("where - Condition as Arrow array, Other as Scalar")
        {
            ...
            auto bool_arrow =
                Array{make_contiguous_array(std::vector<bool>{true, false, true, false, true})};
            ...
            auto expected =
                make_dataframe(default_frame.index(),
                               std::vector{std::vector<int64_t>{1, 123, 3, 123, 5},
        ...
        SECTION("where - Condition as Series, Other as DataFrame")
        {
            ...
            // Condition returns a Series based on column "A": (1,2,3,4,5) > 2 → {false, false,
            // true, true, true}
            DataFrame result = default_frame.where(default_frame["A"] > 2_scalar, other_callable);
            auto      expected =
                make_dataframe(default_frame.index(),
                               std::vector{std::vector<int64_t>{777, 777, 3, 4, 5},
        ...
        SECTION("where - Condition as DataFrame, Other as DataFrame")
        {
            ...
            auto other_df =
                make_dataframe(default_frame.index(),
                               std::vector{std::vector<int64_t>{444, 444, 444, 444, 444},
                                           std::vector<int64_t>{444, 444, 444, 444, 444},
                                           std::vector<int64_t>{444, 444, 444, 444, 444}},
                               default_frame.column_names());
            DataFrame result = default_frame.where(bool_df, other_df);
            // Expected: element-wise, if the corresponding entry in bool_df is true, take the
            // original value; otherwise use 444.
            auto expected =
                make_dataframe(default_frame.index(),
                               std::vector{std::vector<int64_t>{1, 444, 3, 444, 5},
        ...
        SECTION("where - Condition DataFrame with mismatched shape (should throw)")
        {
            ...
            auto wrong_bool_df = make_dataframe(
                wrong_index,
                std::vector{std::vector<bool>{true, false, true},
        ...
        SECTION("where - Other DataFrame with mismatched shape (should throw)")
        {
            ...
            auto wrong_index    = from_range(0, 3);
            auto wrong_other_df = make_dataframe(wrong_index,
                                                 std::vector{std::vector<int64_t>{999, 999, 999},
                                                             std::vector<int64_t>{999, 999, 999},
                                                             std::vector<int64_t>{999, 999, 999}},
                                                 default_frame.column_names());
            // Use a valid condition (from a Series) so that only the "other" DataFrame is
            // mismatched.
...
TEST_CASE("Series Indexing Ops", "[Series][Indexing]")
{
    ...
    SECTION("loc with callable filter")
    {
        ...
        // Assuming that the Series indexing preserves the original index order for the filtered
        // rows
    ...
    SECTION("loc with variant row argument")
    {
        ...
        // Assuming that Series.loc supports a tuple (row, unused) similar to DataFrame.iloc(row,
        // col) Since Series only has one column, we simulate by passing a tuple with the row index
        // and an empty string
