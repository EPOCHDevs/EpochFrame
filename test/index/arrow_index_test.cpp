...
#define TEST_CASE_TYPES epoch_frame::RangeIndex //, arrow::TimestampArray
...
TEMPLATE_TEST_CASE("ArrowIndex - Constructor & Basic Attributes", "[arrow_index][constructor]",
                   TEST_CASE_TYPES)
{
...
TEMPLATE_TEST_CASE("ArrowIndex - NBytes/Empty", "[arrow_index][memory]", TEST_CASE_TYPES)
{
    using CType = typename TestType::value_type;
...
TEMPLATE_TEST_CASE("ArrowIndex - Min/Max/ArgMin/ArgMax", "[arrow_index][reductions]",
                   TEST_CASE_TYPES)
{
...
TEMPLATE_TEST_CASE("ArrowIndex - Equality checks", "[arrow_index][equality]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;
    ...
    SECTION("identical")
    {
        // depends on your implementation. If identical checks same pointer & same name => false
        // here
...
TEMPLATE_TEST_CASE("ArrowIndex - drop(labels)", "[arrow_index][drop]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;
...
TEMPLATE_TEST_CASE("ArrowIndex - delete_/insert", "[arrow_index][delete_insert]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;

    auto idx = epoch_frame::factory::index::make_range(std::vector<CType>{10, 20, 30, 40},
                                                       epoch_frame::MonotonicDirection::Increasing);
...
//------------------------------------------------------------------------------
TEMPLATE_TEST_CASE("ArrowIndex - get_loc, slice_locs", "[arrow_index][search]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;
    ...
    SECTION("slice_locs")
    {
        auto [start, stop, step] =
            idx->slice_locs(Scalar(static_cast<CType>(20)), Scalar(static_cast<CType>(40)));
...
TEMPLATE_TEST_CASE("ArrowIndex - get_loc with duplicates", "[arrow_index][search]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;

    std::vector<CType> data{10, 20, 30, 30, 40, 50};
    auto               arr = epoch_frame::factory::array::make_contiguous_array<CType>(data);
    auto               idx = std::make_shared<TestType>(arr);

    SECTION("get_loc with duplicate value")
    {
        auto loc = idx->get_loc(Scalar(static_cast<CType>(30)));
        REQUIRE(loc.size() == 2);
        REQUIRE(loc[0] == 2);
    ...
    SECTION("slice_locs with duplicates")
    {
        auto [start, stop, step] =
            idx->slice_locs(Scalar(static_cast<CType>(20)), Scalar(static_cast<CType>(40)));
        // Typically [1,5) => includes 20,30,30,40
        REQUIRE(start == 1);
        REQUIRE(stop == 5);
        REQUIRE(step == 1);
    }
...
TEMPLATE_TEST_CASE("ArrowIndex - searchsorted", "[arrow_index][search]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;
    ...
    // Test for left side
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(1)), epoch_frame::SearchSortedSide::Left) ==
            0);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(10)),
                              epoch_frame::SearchSortedSide::Left) == 0);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(11)),
                              epoch_frame::SearchSortedSide::Left) == 1);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(50)),
                              epoch_frame::SearchSortedSide::Left) == 4);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(51)),
                              epoch_frame::SearchSortedSide::Left) == 5);

    // Test for right side
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(1)),
                              epoch_frame::SearchSortedSide::Right) == 0);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(10)),
                              epoch_frame::SearchSortedSide::Right) == 1);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(11)),
                              epoch_frame::SearchSortedSide::Right) == 1);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(50)),
                              epoch_frame::SearchSortedSide::Right) == 5);
    REQUIRE(idx->searchsorted(Scalar(static_cast<CType>(51)),
                              epoch_frame::SearchSortedSide::Right) == 5);
}
...
TEMPLATE_TEST_CASE("ArrowIndex - Set operations", "[arrow_index][setops]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;
...
TEMPLATE_TEST_CASE("ArrowIndex - take/where/putmask", "[arrow_index][filtering]", TEST_CASE_TYPES)
{
    using ArrowType = typename TestType::array_type;
    using CType     = typename TestType::value_type;
    ...
    SECTION("where")
    {
        ...
        auto condRes =
            arrow::compute::CallFunction("greater", {arrow::Datum{arr}, arrow::Datum{20}});
...
TEST_CASE("ArrowIndex Edge Cases - Null pointer construction", "[arrow_index][edge_cases]")
{
    ...
    REQUIRE_THROWS_AS(std::make_shared<RangeIndex>(nullArray, MonotonicDirection::Increasing),
                      std::runtime_error);
    REQUIRE_THROWS_AS(std::make_shared<ObjectIndex>(nullArray), std::runtime_error);
}
...
TEST_CASE("Index - Series and DataFrame assign", "[index][assign]")
{
    ...
    SECTION("DataFrame::assign with matching indices")
    {
        ...
        std::vector<std::vector<int64_t>> data1 = {{1, 2, 3, 4, 5}, {10, 20, 30, 40, 50}};

        auto df1 = make_dataframe<int64_t>(index1, data1, {"col1", "col2"});

        // Create data to assign
        std::vector<std::vector<int64_t>> data2 = {{200, 300, 600}, {2000, 3000, 6000}};
    ...
    SECTION("DataFrame::assign with identical indices")
    {
        ...
        std::vector<std::vector<int64_t>> data2 = {{100, 200, 300, 400, 500},
    ...
    SECTION("DataFrame::assign with DataFrame overload")
    {
        ...
        // Create data to assign with index3 = {"a", "c", "e"}
        std::vector<std::vector<int64_t>> data3 = {{100, 300, 500}, {1000, 3000, 5000}};
