...
//         REQUIRE_THROWS_AS(cal.modify_market_times(epoch_core::MarketTimeType::MarketOpen, Time{12h}), std::runtime_error);
//     }
//
//     SECTION("Cannot delete regular market times") {
//         REQUIRE_THROWS_AS(cal.remove_market_time(epoch_core::MarketTimeType::MarketOpen), std::runtime_error);
//     }
// }
...
//         REQUIRE_THROWS_AS(cal.add_market_time(epoch_core::MarketTimeType::Interruption1, Time{11h, 30min}), std::runtime_error);
//     }
// }
TEST_CASE("market calendar", "[calendar]") {
    ...
    const static MarketCalendarOptions FAKE_CALENDAR{
        ...
        .regular_market_times = RegularMarketTimes{{
            {epoch_core::MarketTimeType::MarketOpen, {MarketTime{Time{11h, 18min}}, {Time{11h, 13min}, std::nullopt, Date{1902y, March, 4d}}}},
            {epoch_core::MarketTimeType::MarketClose, {MarketTime{Time{11h, 45min}}, {Time{11h, 49min}, std::nullopt, Date{1901y, February, 3d}}}},
        }},
        .tz = "Asia/Ulaanbaatar",
        .regular_holidays = std::make_shared<AbstractHolidayCalendar>(AbstractHolidayCalendarData{
            { us.USNewYearsDay, us.Christmas }
        }),
        ...
        .special_opens = {
            {Time{11h, 15min}, std::make_shared<AbstractHolidayCalendar>(AbstractHolidayCalendarData{
                { us.MonTuesThursBeforeIndependenceDay }
            })},
            {Time{23h}, std::make_shared<AbstractHolidayCalendar>(AbstractHolidayCalendarData{
                {nyse.Sept11Anniversary12pmLateOpen2002 }
            }), -1},
        },
        .special_opens_adhoc = {
            {Time{11h, 20min}, factory::index::make_datetime_index({
                "2016-12-13"__date, "2016-12-25"__date
            })},
            {Time{22h}, factory::index::make_datetime_index( {
                 "2016-12-07"__date, "2016-12-09"__date
            }), -1},
        },
        .special_closes = {
            {Time{11h, 30min}, std::make_shared<AbstractHolidayCalendar>(AbstractHolidayCalendarData{
                {us.MonTuesThursBeforeIndependenceDay}
            })},
            {Time{1h}, std::make_shared<AbstractHolidayCalendar>(AbstractHolidayCalendarData{
                {nyse.Sept11Anniversary12pmLateOpen2002}
            }), 1},
        },
        .special_closes_adhoc = {
            {Time{11h, 40min}, factory::index::make_datetime_index({
                "2016-12-14"__date
            })},
            {Time{1h, 5min}, factory::index::make_datetime_index({
                "2016-12-16"__date
            }), 1}
        },
        .interruptions = { }
    ...
    const static MarketCalendarOptions FAKE_BREAK_CALENDAR{
        ...
        .special_opens_adhoc = {
            {Time{10h, 20min}, factory::index::make_datetime_index({
                "2016-12-29"__date
            })}
        },
        .special_closes_adhoc = {
            {Time{10h, 40min}, factory::index::make_datetime_index({
                "2016-12-30"__date
            })}
        },
        .interruptions = { }
    ...
    SECTION("test_get_time") {
        ...
        SECTION("Get current market open time") {
            REQUIRE(cal.get_time(epoch_core::MarketTimeType::MarketOpen).at(0).time == Time{.hour=11h, .minute=13min, .tz=FAKE_CALENDAR.tz});
        }
        ...
        SECTION("Get time on specific date") {
            REQUIRE(cal.get_time_on(epoch_core::MarketTimeType::MarketClose, Date{1900y, January, 1d})->time == Time{.hour=11h, .minute=45min, .tz=FAKE_CALENDAR.tz});
            REQUIRE_FALSE(cal.get_time_on(epoch_core::MarketTimeType::BreakStart, Date{1900y, January, 1d}).has_value());
        }
        ...
        SECTION("Attempt to get non-existent time should throw") {
            ...
            REQUIRE_THROWS_AS(cal.get_time_on(epoch_core::MarketTimeType::Pre, "1900-01-01"__date.date), std::runtime_error);
        }
    }
    ...
    SECTION("test_get_offset") {
        ...
        cal.change_time(epoch_core::MarketTimeType::MarketOpen, {
        {Time{10h}, -1}});
        cal.change_time(epoch_core::MarketTimeType::MarketClose, {
    {Time{10h}, 5}});
    ...
    SECTION("test_special_dates", "[calendar]") {
        ...
        SECTION("Special market open dates") {
            auto special_dates = cal.special_dates(epoch_core::MarketTimeType::MarketOpen, "2016-12-10"__date.date, "2016-12-31"__date.date);
            REQUIRE(special_dates.size() == 1);
            REQUIRE(special_dates.iloc(0).to_datetime() == "2016-12-13 03:20:00"__dt.replace_tz(tz));
        }

        SECTION("Special market open dates including holidays") {
            auto special_dates = cal.special_dates(epoch_core::MarketTimeType::MarketOpen, "2016-12-10"__date.date, "2016-12-31"__date.date, false);
            REQUIRE(special_dates.size() == 2);
            REQUIRE(special_dates.iloc(0).to_datetime() == "2016-12-13 03:20:00"__dt.replace_tz(tz));
            REQUIRE(special_dates.iloc(1).to_datetime() == "2016-12-25 03:20:00"__dt.replace_tz(tz));
        }
    }
    ...
    SECTION("test_days_at_time") {
        ...
        chicago.add_time(epoch_core::MarketTimeType::InternalUseOnly, {MarketTime{Time{10h, 30min}, -1}});

        // Helper function to test days_at_time with various combinations
        auto test_days_at_time = []<typename T>(const Date& day,
                                    std::optional<int64_t> day_offset,
                                    T const& time_offset,
                                    MarketCalendar& cal,
                                    const std::string& expected) {
            // Create days index
            auto days = factory::index::make_datetime_index({DateTime{.date = day, .tz=cal.tz()} });
            auto expected_dt = DateTime::from_str(expected).replace_tz(cal.tz());
            DYNAMIC_SECTION(day << " expected " << expected_dt << " with time_offset " << time_offset) {
                Series result;
        ...
        test_days_at_time(
            "2004-04-05"__date.date,
            std::nullopt,
            epoch_core::MarketTimeType::InternalUseOnly,
            chicago,
            "2004-04-04 10:30:00"
        );
    ...
    SECTION("test_schedule") {
        ...
        REQUIRE(cal.close_time().front().time == Time{.hour=11h, .minute=49min, .tz=cal.tz()});

        SECTION("Datetime calendar") {
            auto index = factory::index::make_datetime_index({
                                "2016-12-01"__date, "2016-12-02"__date
                });

            std::vector market_open{"2016-12-01 03:13:00"__dt.replace_tz("UTC"), "2016-12-02 03:13:00"__dt.replace_tz("UTC")};
            std::vector market_close{"2016-12-01 03:49:00"__dt.replace_tz("UTC"), "2016-12-02 03:49:00"__dt.replace_tz("UTC")};
            auto expected = make_dataframe(index, std::vector{market_open, market_close}, {"MarketOpen", "MarketClose"});
            ...
            SECTION("Series calendar at loc 0") {
                index = factory::index::make_object_index(std::vector<std::string>{"MarketOpen", "MarketClose"});
                auto series_arr = std::vector{ "2016-12-01 03:13:00"__dt.replace_tz("UTC"), "2016-12-01 03:49:00"__dt.replace_tz("UTC")};
            ...
            SECTION("Series calendar at loc -1") {
                index = factory::index::make_object_index(std::vector<std::string>{"MarketOpen", "MarketClose"});
                auto series_arr = std::vector{ "2016-12-30 03:13:00"__dt.replace_tz("UTC"), "2016-12-30 03:49:00"__dt.replace_tz("UTC")};
                auto expected_series = make_series(index, series_arr);
            ...
            SECTION("one day calendar") {
                index = factory::index::make_datetime_index(std::vector<DateTime>{"2016-12-01"__date});
                ...
                expected = make_dataframe(index, std::vector{market_open, market_close}, {"MarketOpen", "MarketClose"});
            ...
            SECTION("different time zone") {
                index = factory::index::make_datetime_index(std::vector<DateTime>{"2016-12-01"__date});
                market_open = std::vector{ "2016-11-30 22:13:00"__dt.replace_tz("US/Eastern") + TimeDelta{{.hours = 5}}};
                market_close = std::vector{"2016-11-30 22:49:00"__dt.replace_tz("US/Eastern") + TimeDelta{{.hours = 5}}};
                expected = make_dataframe(index, std::vector{market_open, market_close}, {"MarketOpen", "MarketClose"});
    ...
    SECTION("test_custom_schedule") {
        ...
        auto expected_cols = std::vector<std::string>{"MarketOpen", "BreakStart", "BreakEnd", "MarketClose"};
        ...
        REQUIRE(dec_29.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            "2016-12-29 15:20:00.000000000Z",
            "2016-12-29 15:20:00.000000000Z",
            "2016-12-29 16:00:00.000000000Z",
            "2016-12-29 17:00:00.000000000Z"
        });

        // Special market_close should take effect on 12/30
        auto dec_30 = schedule.loc(Scalar("2016-12-30"__date));
        REQUIRE(dec_30.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            "2016-12-30 14:30:00.000000000Z",
            "2016-12-30 15:00:00.000000000Z",
            "2016-12-30 15:40:00.000000000Z",
            "2016-12-30 15:40:00.000000000Z"
        });

        // Test custom start and end
        auto schedule_custom_start_end = cal.schedule("2016-12-23"__date.date, "2016-12-31"__date.date, {
        ...
        REQUIRE(dec_30.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            ...
            "2016-12-30 14:30:00.000000000Z",
            "2016-12-30 15:00:00.000000000Z",
            "2016-12-30 16:00:00.000000000Z"
        });

        // Test custom market times
        auto schedule_custom_market_times = cal.schedule("2016-12-23"__date.date, "2016-12-31"__date.date, {
            .market_times = std::vector<epoch_core::MarketTimeType>{epoch_core::MarketTimeType::Post, epoch_core::MarketTimeType::Pre}
        });
        ...
        REQUIRE(dec_29.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            "2016-12-29 18:00:00.000000000Z",
            "2016-12-29 14:00:00.000000000Z"
        });

        dec_30 = schedule_custom_market_times.loc(Scalar("2016-12-30"__date));
        REQUIRE(dec_30.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            "2016-12-30 18:00:00.000000000Z",
            "2016-12-30 14:00:00.000000000Z"
        });

        // Only adjust column itself (force_special_times=false)
        auto schedule_no_force = cal.schedule("2016-12-23"__date.date, "2016-12-31"__date.date, {
            .force_special_times = epoch_core::BooleanEnum::False
        });
        expected_cols = std::vector<std::string>{"MarketOpen", "BreakStart", "BreakEnd", "MarketClose"};
        ...
        REQUIRE(dec_29.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            "2016-12-29 15:20:00.000000000Z",
            "2016-12-29 15:00:00.000000000Z",
            "2016-12-29 16:00:00.000000000Z",
            "2016-12-29 17:00:00.000000000Z"
        });
        ...
        auto schedule_ignore_special = cal.schedule("2016-12-23"__date.date, "2016-12-31"__date.date, {
        ...
        REQUIRE(dec_29.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            "2016-12-29 14:30:00.000000000Z",
            "2016-12-29 15:00:00.000000000Z",
            "2016-12-29 16:00:00.000000000Z",
            "2016-12-29 17:00:00.000000000Z"
        });
        ...
        REQUIRE(dec_30.cast(arrow::utf8()).contiguous_array().to_vector<std::string>() == std::vector<std::string>{
            ...
            "2016-12-30 15:00:00.000000000Z",
            "2016-12-30 16:00:00.000000000Z",
            "2016-12-30 17:00:00.000000000Z"
        });
    }
    ...
    SECTION("test_schedule_w_breaks") {
        ...
        // Check market times
        REQUIRE(cal.get_time(epoch_core::MarketTimeType::MarketOpen).front().time == Time{.hour = 9h, .minute = 30min, .tz = cal.tz()});
        REQUIRE(cal.get_time(epoch_core::MarketTimeType::MarketClose).front().time == Time{.hour = 12h, .minute = 0min, .tz = cal.tz()});
        REQUIRE(cal.get_time(epoch_core::MarketTimeType::BreakStart).front().time == Time{.hour = 10h, .minute = 0min, .tz = cal.tz()});
        REQUIRE(cal.get_time(epoch_core::MarketTimeType::BreakEnd).front().time == Time{.hour = 11h, .minute = 0min, .tz = cal.tz() });
        
        // Create expected dataframe for Dec 1-2
        auto dates_index = factory::index::make_datetime_index({
            "2016-12-01"__date, "2016-12-02"__date
        }, "", "");
        
        std::vector market_open{
            "2016-12-01 14:30:00"__dt.replace_tz("UTC"),
            "2016-12-02 14:30:00"__dt.replace_tz("UTC")
        };
        std::vector market_close{
            "2016-12-01 17:00:00"__dt.replace_tz("UTC"),
            "2016-12-02 17:00:00"__dt.replace_tz("UTC")
        };
        std::vector break_start{
            "2016-12-01 15:00:00"__dt.replace_tz("UTC"),
            "2016-12-02 15:00:00"__dt.replace_tz("UTC")
        };
        std::vector break_end{
        ...
        auto expected = make_dataframe(
            dates_index, 
            std::vector{market_open, break_start, break_end, market_close}, 
        ...
        // Check first day
        auto index = factory::index::make_object_index(std::vector<std::string>{"MarketOpen", "BreakStart", "BreakEnd", "MarketClose"});
        auto expected_first_day = make_series(
            index, 
            std::vector{
                "2016-12-01 14:30:00"__dt.replace_tz("UTC"),
                "2016-12-01 15:00:00"__dt.replace_tz("UTC"),
        ...
        auto expected_special_open = make_series(
            index, 
            std::vector{
        ...
        auto expected_special_close = make_series(
            index, 
            std::vector{
                "2016-12-30 14:30:00"__dt.replace_tz("UTC"),
        ...
        dates_index = factory::index::make_datetime_index({
            "2016-12-28"__date
        }, "", "");
        
        market_open = std::vector{
            "2016-12-28 14:30:00"__dt.replace_tz("America/New_York")
        };
        market_close = std::vector{
            "2016-12-28 17:00:00"__dt.replace_tz("America/New_York")
        };
        break_start = std::vector{
            "2016-12-28 15:00:00"__dt.replace_tz("America/New_York")
        };
        break_end = std::vector{
            "2016-12-28 16:00:00"__dt.replace_tz("America/New_York")
        };
    ...
    SECTION("test_schedule_w_times") {
        ...
        REQUIRE(cal.get_time(epoch_core::MarketTimeType::MarketOpen).front().time == Time{.hour = 12h, .minute = 12min, .tz = cal.tz()});
        REQUIRE(cal.get_time(epoch_core::MarketTimeType::MarketClose).front().time == Time{.hour = 13h, .minute = 13min, .tz = cal.tz()});
        ...
        // Check first day
        auto index = factory::index::make_object_index(std::vector<std::string>{"MarketOpen", "MarketClose"});
        auto expected_first_day = make_series(
            index, 
            std::vector{
                "2016-12-01 04:12:00"__dt.replace_tz("UTC"),
        ...
        // Check last day
        auto expected_last_day = make_series(
            index, 
            std::vector{
                "2016-12-30 04:12:00"__dt.replace_tz("UTC"),
    ...
    //     auto july_2nd_open = "2012-07-02 11:13:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto july_3rd_open = "2012-07-03 11:15:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto july_4th_open = "2012-07-04 11:13:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    ...
    //     // Convert expected timestamps to UTC
    //     auto dec_12th_open = "2016-12-12 11:13:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto dec_13th_open = "2016-12-13 11:20:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto dec_14th_open = "2016-12-14 11:13:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    ...
    //     // Convert expected timestamps to UTC
    //     auto july_2nd_close = "2012-07-02 11:49:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto july_3rd_close = "2012-07-03 11:30:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto july_4th_close = "2012-07-04 11:49:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    ...
    //     // Convert expected timestamps to UTC
    //     auto dec_13th_close = "2016-12-13 11:49:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto dec_14th_close = "2016-12-14 11:40:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
    //     auto dec_15th_close = "2016-12-15 11:49:00"__dt.replace_tz("Asia/Ulaanbaatar").tz_convert("UTC");
