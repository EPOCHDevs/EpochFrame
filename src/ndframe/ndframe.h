...
namespace epoch_frame
{
    ...
    template <class ChildType, class ArrowType> class NDFrame
    {
      public:
        using ArrowPtrType = std::shared_ptr<ArrowType>;
        using AggType =
            std::conditional_t<std::is_same_v<ArrowType, arrow::Table>, class Series, class Scalar>;
        ...
        ChildType round_to_multiple(
            double                    multiple   = 1,
            arrow::compute::RoundMode round_mode = arrow::compute::RoundMode::HALF_TO_EVEN) const;

        ChildType round_binary(
            arrow::compute::RoundMode round_mode = arrow::compute::RoundMode::HALF_TO_EVEN) const;
        ...
        //--------------------------------------------------------------------------
        ChildType cumulative_sum(bool                         skip_nulls = true,
                                 std::optional<double> const& start      = std::nullopt) const;

        ChildType cumulative_prod(bool                         skip_nulls = true,
                                  std::optional<double> const& start      = std::nullopt) const;

        ChildType cumulative_mean(bool                         skip_nulls = true,
                                  std::optional<double> const& start      = std::nullopt) const;

        ChildType cumulative_max(bool                         skip_nulls = true,
                                 std::optional<double> const& start      = std::nullopt) const;

        ChildType cumulative_min(bool                         skip_nulls = true,
                                 std::optional<double> const& start      = std::nullopt) const;
        ...
        ChildType reindex(IndexPtr const& index, const Scalar& fillValue = Scalar{}) const;
        ...
        ChildType cast(arrow::DataTypePtr const& to_type,
                       bool safe = true) const; // expose target type as a string
        ...
        ChildType sort_values(std::vector<std::string> const& by, bool place_na_last = true,
         ...
         */
        ChildType map(const std::function<Scalar(const Scalar&)>& func,
                      bool                                        ignore_nulls = false) const;

        //--------------------------------------------------------------------------
        // 14) Aggregation
        //--------------------------------------------------------------------------
        AggType agg(AxisType axis, std::string const& agg, bool skip_null = true) const;
        AggType agg(AxisType axis, std::string const& agg, bool skip_null,
        ...
        AggType   product(AxisType axis = AxisType::Row, bool skip_null = true,
                          uint32_t min_count = 0) const;
        AggType   quantile(arrow::compute::QuantileOptions const& options,
                           AxisType                               axis = AxisType::Row) const;
        AggType   stddev(arrow::compute::VarianceOptions const& options,
                         AxisType                               axis = AxisType::Row) const;
        AggType   sum(AxisType axis = AxisType::Row, bool skip_null = true) const;
        AggType   tdigest(arrow::compute::TDigestOptions const& options,
                          AxisType                              axis = AxisType::Row) const;
        AggType   variance(arrow::compute::VarianceOptions const& options,
                           AxisType                               axis = AxisType::Row) const;

        bool equals(ChildType const&,
