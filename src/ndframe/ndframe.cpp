...
//////////////////////////////////////////////////////////////////////////
    ...
    NDFrame<ChildType, ArrowType>::NDFrame()
            : NDFrame(factory::index::from_range(0), factory::table::make_empty_table_or_array<ArrowType>()) {}
    ...
    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType> & NDFrame<ChildType, ArrowType>::operator=(NDFrame const &other) = default;

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType> & NDFrame<ChildType, ArrowType>::operator=(NDFrame &&other) noexcept = default;

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::~NDFrame() = default;

//////////////////////////////////////////////////////////////////////////
/// GENERAL ATTRIBUTES
//////////////////////////////////////////////////////////////////////////
    template<class ChildType, class ArrowType>
    Shape2D NDFrame<ChildType, ArrowType>::shape() const {
        if constexpr (std::is_same_v<ArrowType, arrow::Table>) {
            return { static_cast<unsigned long>(m_table->num_rows()),
                     static_cast<unsigned long>(m_table->num_columns()) };
        } else if constexpr (std::is_same_v<ArrowType, arrow::ChunkedArray>) {
            return { static_cast<unsigned long>(m_table->length()),
                     static_cast<unsigned long>(1) };
        }
    }

    template<class ChildType, class ArrowType>
    bool NDFrame<ChildType, ArrowType>::empty() const {
        return size() == 0;
    }

    template<class ChildType, class ArrowType>
    uint64_t NDFrame<ChildType, ArrowType>::size() const {
        return m_index->size();
    }
...
//////////////////////////////////////////////////////////////////////////
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::abs() const {
...
//////////////////////////////////////////////////////////////////////////
    ...
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::exp() const {
    ...
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::ln() const {
...
//////////////////////////////////////////////////////////////////////////
    ...
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cos() const {
        return from_base(m_arithOp->cos());
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::sin() const {
        return from_base(m_arithOp->sin());
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::tan() const {
...
//////////////////////////////////////////////////////////////////////////
    ...
    template<class ChildType, class ArrowType>
    arrow::compute::CumulativeOptions MakeCumulativeOptions(bool skip_nulls, std::optional<double> const& start) {
        return start ? arrow::compute::CumulativeOptions{ *start, skip_nulls } : arrow::compute::CumulativeOptions{ skip_nulls };
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cumulative_sum(bool skip_nulls, std::optional<double> const& start) const {
        return from_base(m_arithOp->cumulative_sum(MakeCumulativeOptions<ChildType, ArrowType>(skip_nulls, start)));
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cumulative_prod(bool skip_nulls, std::optional<double> const& start) const {
        return from_base(m_arithOp->cumulative_prod(MakeCumulativeOptions<ChildType, ArrowType>(skip_nulls, start)));
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cumulative_max(bool skip_nulls, std::optional<double> const& start) const {
        return from_base(m_arithOp->cumulative_max(MakeCumulativeOptions<ChildType, ArrowType>(skip_nulls, start)));
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cumulative_min(bool skip_nulls, std::optional<double> const& start) const {
        return from_base(m_arithOp->cumulative_min(MakeCumulativeOptions<ChildType, ArrowType>(skip_nulls, start)));
    }

    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cumulative_mean(bool skip_nulls, std::optional<double> const& start) const {
        return from_base(m_arithOp->cumulative_mean(MakeCumulativeOptions<ChildType, ArrowType>(skip_nulls, start)));
    }
...
//////////////////////////////////////////////////////////////////////////
    ...
    ChildType NDFrame<ChildType, ArrowType>::tail(uint64_t n) const {
        return iloc(UnResolvedIntegerSliceBound{.start=std::max(static_cast<int64_t>(size())-static_cast<int64_t>(n), 0L)});
    }
    ...
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::reindex(IndexPtr const &index, const Scalar &fillValue) const {
        return from_base(
                index,
...
//////////////////////////////////////////////////////////////////////////
    ...
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::cast(arrow::DataTypePtr const &to_type, bool safe) const {
    ...
    //--------------------------------------------------------------------------
    template<class ChildType, class ArrowType>
    ChildType NDFrame<ChildType, ArrowType>::drop_null(DropMethod how, AxisType axis, std::vector<std::string> const &subset, bool ignore_index) const {
    ...
    template<class ChildType, class ArrowType>
    typename NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::all(AxisType axis, bool skip_null) const {
        return m_agg->all(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::any(AxisType axis, bool skip_null) const {
        return m_agg->any(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::approximate_median(AxisType axis, bool skip_null) const {
        return m_agg->approximate_median(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::count_all(AxisType axis) const {
    ...
    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::count_valid(AxisType axis) const {
        arrow::compute::CountOptions options;
    ...
    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::count_null(AxisType axis) const {
        arrow::compute::CountOptions options;
    ...
    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::first(AxisType axis, bool skip_null) const {
        return m_agg->first(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::last(AxisType axis, bool skip_null) const {
        return m_agg->last(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::index(Scalar const &scalar, AxisType axis) const {
    ...
    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::max(AxisType axis, bool skip_null) const {
        return m_agg->max(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::min(AxisType axis, bool skip_null) const {
        return m_agg->min(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::mean(AxisType axis, bool skip_null) const {
    ...
    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::product(AxisType axis, bool skip_null, uint32_t min_count) const {
        arrow::compute::ScalarAggregateOptions options{skip_null, min_count};
        return m_agg->product(options, axis).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::quantile(arrow::compute::QuantileOptions const &options, AxisType axis) const {
        return m_agg->quantile(options, axis).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::stddev(arrow::compute::VarianceOptions const &options, AxisType axis) const {
        return m_agg->stddev(options, axis).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::sum(AxisType axis, bool skip_null) const {
        return m_agg->sum(axis, skip_null).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::tdigest(arrow::compute::TDigestOptions const &options, AxisType axis) const {
        return m_agg->tdigest(options, axis).as<AggType>();
    }

    template<class ChildType, class ArrowType>
    NDFrame<ChildType, ArrowType>::AggType NDFrame<ChildType, ArrowType>::variance(arrow::compute::VarianceOptions const &options, AxisType axis) const {
