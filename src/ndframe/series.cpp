...
namespace epoch_frame
{
    ...
    Series::Series(IndexPtr index, arrow::ChunkedArrayPtr array,
                   const std::optional<std::string>& name)
        : NDFrame<Series, arrow::ChunkedArray>(index, array), m_name(name)
    {
    }
    ...
    Series::Series(arrow::ArrayPtr const& data, std::optional<std::string> const& name)
        : Series(factory::index::from_range(0, data->length()), data, name)
    {
    }
    Series::Series(arrow::ScalarPtr const& data, IndexPtr const& index,
                   std::optional<std::string> const& name)
        : Series(index, AssertResultIsOk(arrow::MakeArrayFromScalar(*data, index->size())), name)
    {
    }

    Series::Series(arrow::ChunkedArrayPtr const& data, std::optional<std::string> const& name)
        : Series(factory::index::from_range(0, data->length()), data, name)
    {
    }
    ...
    Series Series::n_largest(int64_t n) const
    {
        ...
        auto sorted_indices = arrow_utils::call_unary_compute_contiguous_array(
            contiguous_array().value(), "array_sort_indices", &options);
    ...
    DataFrame Series::transpose(IndexPtr const& new_index) const
    {
        ...
        for (size_t i = 0; i < m_index->size(); ++i)
        {
            fields.push_back(arrow::field(
                m_index->array().value()->GetScalar(i).MoveValueUnsafe()->ToString(), type));
            columns.push_back(AssertResultIsOk(arrow::MakeArrayFromScalar(*iloc(i).value(), 1)));
        }
        auto arr = arrow::Table::Make(arrow::schema(fields), columns);
...
#pragma GCC diagnostic ignored "-Wstringop-overflow"
        ...
        header.push_back(
            std::format("{}({})", df.m_name.value_or(""), df.m_table->type()->ToString()));
        table.add_row(header);
        for (size_t i = 0; i < df.m_index->size(); ++i)
        {
            const auto index = tabulate::Table::Row_t::value_type{
...
#pragma GCC diagnostic pop
    ...
    Series::rolling_agg(window::RollingWindowOptions const& options) const
    {
        return {std::make_unique<window::RollingWindow>(options), *this};
    }

    ApplySeriesRollingWindowOperations
    Series::rolling_apply(window::RollingWindowOptions const& options) const
    {
        return {std::make_unique<window::RollingWindow>(options), *this};
    }

    AggRollingWindowOperations<false>
    Series::expanding_agg(window::ExpandingWindowOptions const& options) const
    {
        return {std::make_unique<window::ExpandingWindow>(options), *this};
    }

    ApplySeriesRollingWindowOperations
    Series::expanding_apply(window::ExpandingWindowOptions const& options) const
    {
        return {std::make_unique<window::ExpandingWindow>(options), *this};
    }
    ...
    Series Series::assign(IndexPtr const& indices, arrow::ChunkedArrayPtr const& arr) const
    {
        ...
        AssertFromFormat(arr->type()->Equals(m_table->type()),
                         "Array and series must have the same type. {} != {}.",
        ...
        for (size_t i = 0; i < size(); ++i)
        {
            ...
            if (indices->contains(index_scalar))
            {
                ...
                AssertFalseFromStream(loc.empty(), "assign: index not found");
                for (auto l : loc)
                {
                    scalars.push_back(arr->GetScalar(l).MoveValueUnsafe());
                    ++inserts;
                }
            }
            else
        ...
        if (inserts != indices->size())
        {
            ...
            ThrowException("Indices must be a subset of the original index, found {} elements in "
                           "the difference\n{}",
