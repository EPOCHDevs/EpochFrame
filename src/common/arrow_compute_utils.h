...
namespace epoch_frame::arrow_utils
{
     ...
     */
    inline arrow::Datum call_compute(const std::vector<arrow::Datum>&       inputs,
                                     ...
                                     const arrow::compute::FunctionOptions* options = nullptr)
    {
        ...
        if (!result.ok())
        {
            throw std::runtime_error(std::format("CallFunction({}) failed: {}", function_name,
                                                 result.status().ToString()));
        }
        return *result;
    }

    inline arrow::Datum call_unary_compute(const arrow::Datum&                    input,
                                           const std::string&                     function_name,
                                           const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    inline arrow::TablePtr
    call_compute_table(const std::vector<arrow::Datum>& inputs, const std::string& function_name,
                       const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    inline arrow::TablePtr
    call_unary_compute_table(const arrow::Datum& input, const std::string& function_name,
                             const arrow::compute::FunctionOptions* options = nullptr)
    {
        arrow::Datum result;
        try
        ...
        {
            throw std::runtime_error(std::format("Failed to call unary compute function {}: {}\n{}",
                                                 function_name, e.what(), result.ToString()));
        }
    }

    inline arrow::ArrayPtr
    call_unary_compute_contiguous_array(const arrow::Datum& input, const std::string& function_name,
                                        const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    call_unary_compute_array(const arrow::Datum& input, const std::string& function_name,
                             const arrow::compute::FunctionOptions* options = nullptr)
    {
        if (input.is_chunked_array())
        {
            ...
            {
                arrays.push_back(
                    call_unary_compute_contiguous_array(chunk, function_name, options));
            }
    ...
    TableOrArray
    call_unary_compute_table_or_array(const TableOrArray& input, const std::string& function_name,
                                      const arrow::compute::FunctionOptions* options = nullptr);

    inline arrow::ChunkedArrayPtr
    call_compute_array(const std::vector<arrow::Datum>& inputs, const std::string& function_name,
                       const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    inline arrow::ArrayPtr
    call_compute_contiguous_array(const std::vector<arrow::Datum>&       inputs,
                                  const std::string&                     function_name,
                                  const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    inline TableOrArray
    call_compute_table_or_array(TableOrArray const& input, const std::vector<arrow::Datum>& others,
                                const std::string&                     function_name,
                                const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    ArrowScalarType call_compute_scalar_as(const std::vector<arrow::Datum>&       inputs,
                                           const std::string&                     function_name,
                                           const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    template <typename ArrowScalarType>
    ArrowScalarType call_compute_scalar_as(const arrow::Datum&                    input,
                                           const std::string&                     function_name,
                                           const arrow::compute::FunctionOptions* options = nullptr)
    {
        return call_compute_scalar_as<ArrowScalarType>(std::vector<arrow::Datum>{input},
                                                       function_name, options);
    }

    template <typename ArrowScalarType>
    ArrowScalarType
    call_unary_compute_scalar_as(const arrow::Datum& input, const std::string& function_name,
                                 const arrow::compute::FunctionOptions* options = nullptr)
    {
    ...
    template <typename ArrowScalarType>
    ArrowScalarType call_unary_agg_compute_as(const arrow::Datum& input,
                                              const std::string&  function_name,
    ...
    inline arrow::ScalarPtr call_unary_agg_compute(const arrow::Datum& input,
                                                   const std::string&  function_name,
                                                   bool skip_nulls = true, uint32_t min_count = 0)
    {
        arrow::compute::ScalarAggregateOptions options{skip_nulls, min_count};
    ...
    template <typename ArrayType> arrow::ChunkedArrayPtr call_cast_array(const arrow::Datum& array)
    {
        ...
        if (!casted.ok())
        {
            throw std::runtime_error(std::format("Failed to cast array to {}: {}", type->ToString(),
    ...
    template <typename ArrowType>
        requires(std::is_same_v<ArrowType, arrow::Array> ||
                 std::is_same_v<ArrowType, arrow::ChunkedArray> ||
                 std::is_same_v<ArrowType, arrow::Table> ||
                 std::is_same_v<ArrowType, arrow::RecordBatch>)
    std::shared_ptr<ArrowType> slice_array(const std::shared_ptr<ArrowType>& array, size_t start,
                                           size_t length, int64_t step = 1)
    {
        ...
        for (uint64_t i = 0; i < length; i++)
        {
            ...
            index_builder.UnsafeAppend(
                static_cast<uint64_t>(static_cast<int64_t>(start) + i * step));
        }
    ...
    arrow::ScalarPtr cov(const arrow::ChunkedArrayPtr& array, const arrow::ChunkedArrayPtr& other,
                         std::optional<int64_t> min_periods = std::nullopt, int64_t ddof = 1);
    ...
    arrow::TablePtr apply_function_to_table(
        ...
        bool                                                                 merge_chunks = true);

    inline arrow::Datum call_compute_replace_with_mask(const arrow::Datum& input,
    ...
    inline arrow::Datum call_compute_fill_null(const arrow::Datum& input,
                                               const arrow::Datum& replacement)
    {
        return call_compute_replace_with_mask(
            input, AssertResultIsOk(arrow::compute::IsNull(input)), replacement);
    }

    inline arrow::TablePtr call_compute_fill_null_table(const arrow::TablePtr& table,
                                                        const arrow::Datum&    replacement)
    {
    ...
    inline int64_t years(const arrow::TimestampScalar& scalar1,
                         const arrow::TimestampScalar& scalar2)
    {
        return static_cast<int64_t>(static_cast<int32_t>(get_year(scalar1)) -
                                    static_cast<int32_t>(get_year(scalar2)));
    }

    inline int64_t months(const arrow::TimestampScalar& scalar1,
                          const arrow::TimestampScalar& scalar2)
    {
        return static_cast<int64_t>(static_cast<uint32_t>(get_month(scalar1))) -
               static_cast<int64_t>(static_cast<uint32_t>(get_month(scalar2)));
    }
    ...
    inline int64_t seconds(const arrow::TimestampScalar& scalar1,
                           const arrow::TimestampScalar& scalar2)
    {
    ...
    inline int64_t microseconds(const arrow::TimestampScalar& scalar1,
                                const arrow::TimestampScalar& scalar2)
    {
        return std::chrono::duration_cast<std::chrono::microseconds>(duration(scalar1, scalar2))
            .count();
    }
    ...
    inline int64_t nanoseconds(const arrow::TimestampScalar& scalar1,
                               const arrow::TimestampScalar& scalar2)
    {
        return std::chrono::duration_cast<std::chrono::nanoseconds>(duration(scalar1, scalar2))
            .count();
    }
