...
std::shared_ptr<arrow::io::RandomAccessFile> get_input_stream(const std::string& path) {
    if (is_s3_path(path)) {
        ...
        s3_options.log_level = logLevel ? static_cast<arrow::fs::S3LogLevel>(atoi(logLevel)) : arrow::fs::S3LogLevel::Error;
        ...
        if (aws_access_key && aws_secret_key && aws_region) {
            SPDLOG_INFO("Found AWS credentials in environment variables");
            options.ConfigureAccessKey(aws_access_key, aws_secret_key);
        } else {
            SPDLOG_INFO("Using default AWS credentials");
        ...
        if (!s3fs_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to create S3 file system: {}.", s3fs_result.status().ToString()));
        }
        
        auto s3fs = s3fs_result.ValueOrDie();
        
        // Parse the S3 URI to get bucket and key
        std::string s3_uri = path.substr(5);  // Remove "s3://" prefix
        size_t first_slash = s3_uri.find('/');
        if (first_slash == std::string::npos) {
            throw std::runtime_error(fmt::format("Invalid S3 URI format: {}", path));
        }
        
        std::string bucket = s3_uri.substr(0, first_slash);
        std::string key = s3_uri.substr(first_slash + 1);
        
        // Open the file
        auto file_result = s3fs->OpenInputFile(bucket + "/" + key);
        if (!file_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to open S3 file: {}.", file_result.status().ToString()));
        }
        ...
        if (!file_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to open local file: {}.", file_result.status().ToString()));
        }
...
std::shared_ptr<arrow::io::OutputStream> get_output_stream(const std::string& path) {
    if (is_s3_path(path)) {
        ...
        if (!file_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to open S3 file for writing: {}.", file_result.status().ToString()));
        }
        ...
        if (!file_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to open local file for writing: {}.", file_result.status().ToString()));
        }
    ...
    if (index_column) {
        ...
        if (index_pos != -1) {
            ...
            if (chunked_index->num_chunks() == 1) {
                ...
                if (!result.ok()) {
                    throw std::runtime_error(fmt::format("Failed to concatenate index chunks: {}", result.status().ToString()));
                }
            ...
            if (!result.ok()) {
                throw std::runtime_error(fmt::format("Failed to remove index column: {}", result.status().ToString()));
            }
...
DataFrame read_csv(const std::string& csv_content, const CSVReadOptions& options) {
    ...
    if (!csv_reader_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to create CSV reader: {}", csv_reader_result.status().ToString()));
    }
    ...
    if (!table_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to read CSV data: {}", table_result.status().ToString()));
    }
...
void write_csv(const FrameOrSeries& data, std::string& output, const CSVWriteOptions& options) {
    ...
    if (!buffer_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to allocate buffer: {}", buffer_result.status().ToString()));
    }
    ...
    if (!status.ok()) {
        throw std::runtime_error(fmt::format("Failed to write CSV: {}", status.ToString()));
    }
    
    // Finish writing and get the buffer
    status = output_stream->Close();
    if (!status.ok()) {
        throw std::runtime_error(fmt::format("Failed to close output stream: {}", status.ToString()));
    }
...
DataFrame read_json(const std::string& json_content, const JSONReadOptions& options) {
    // Create a DataFrame object to hold the result
    DataFrame df;
    
    try {
        // Parse the JSON content to the DataFrame object using Glaze
        auto err = glz::read_json(df, json_content);
        if (err) {
            throw std::runtime_error(fmt::format("Failed to parse JSON: {}", glz::format_error(err, json_content)));
        }
        
        // Extract index column if specified and not already handled
        if (options.index_column && *options.index_column != "index") {
            // If a different index column was specified, reindex the DataFrame
            auto [new_table, extracted_index] = extract_index_column(df.table(), options.index_column);
            if (extracted_index) {
                auto index = factory::index::make_index(extracted_index, std::nullopt, "");
                return DataFrame(index, new_table);
            }
        }
        
        return df;
    } catch (const std::exception& e) {
        throw std::runtime_error(fmt::format("Failed to read JSON: {}", e.what()));
    }
}
...
DataFrame read_json_file(const std::string& file_path, const JSONReadOptions& options) {
    try {
        ...
        int64_t file_size = 0;
        auto result = input->GetSize();
        if (!result.ok()) {
            throw std::runtime_error(fmt::format("Failed to get file size: {}", result.status().ToString()));
        }
        file_size = result.ValueOrDie();
        
        std::string json_content;
        json_content.resize(file_size);
        auto bytes_read = input->Read(file_size, reinterpret_cast<void*>(json_content.data()));
        if (!bytes_read.ok()) {
            throw std::runtime_error(fmt::format("Failed to read file: {}", bytes_read.status().ToString()));
        }
        
        // Parse the JSON content
        return read_json(json_content, options);
    } catch (const std::exception& e) {
        throw std::runtime_error(fmt::format("Failed to read JSON file: {}", e.what()));
    }
}

void write_json(const FrameOrSeries& data, std::string& output, const JSONWriteOptions& options) {
    try {
        // Write DataFrame or Series to JSON using Glaze
        if (data.is_frame()) {
            // Handle DataFrame
            auto df = data.frame();
            
            if (options.orient_records) {
                // Records-oriented JSON (each row is an object)
                auto result = glz::write_json(df);
                if (!result) {
                    throw std::runtime_error("Failed to write JSON: Glaze serialization error");
                }
            ...
            auto result = glz::write_json(series);
            if (!result) {
                throw std::runtime_error("Failed to write JSON: Glaze serialization error");
            }
            output = std::move(result.value());
        ...
        throw std::runtime_error(fmt::format("Failed to write JSON: {}", e.what()));
    }
}
...
void write_json_file(const FrameOrSeries& data, const std::string& file_path, const JSONWriteOptions& options) {
    try {
        ...
        write_json(data, json_output, options);
        
        // Get output stream for the file
        auto output_stream = get_output_stream(file_path);
        
        // Write the JSON string to the file
        auto status = output_stream->Write(json_output.data(), json_output.size());
        if (!status.ok()) {
            throw std::runtime_error(fmt::format("Failed to write JSON to file: {}", status.ToString()));
        }
        ...
        throw std::runtime_error(fmt::format("Failed to write JSON file: {}", e.what()));
    }
}
...
DataFrame read_parquet(const std::string& file_path, const ParquetReadOptions& options) {
    ...
    if (!reader_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to open Parquet file: {}", reader_result.status().ToString()));
    }
    ...
    if (options.columns) {
        ...
        if (!status.ok()) {
            throw std::runtime_error(fmt::format("Failed to read Parquet table columns: {}", status.ToString()));
        }
    } else {
        auto status = parquet_reader->ReadTable(&table);
        if (!status.ok()) {
            throw std::runtime_error(fmt::format("Failed to read Parquet table: {}", status.ToString()));
        }
    }
...
void write_parquet(const FrameOrSeries& data, const std::string& file_path, const ParquetWriteOptions& options) {
    ...
    if (!status.ok()) {
        throw std::runtime_error(fmt::format("Failed to write Parquet: {}", status.ToString()));
    }
...
DataFrame read_buffer(const std::shared_ptr<arrow::Buffer>& buffer, const BinaryReadOptions& options) {
    ...
    if (!reader_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to open IPC stream reader: {}", reader_result.status().ToString()));
    }
    ...
    if (!batches_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to read record batches: {}", batches_result.status().ToString()));
    }
    ...
    if (!table_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to convert record batches to table: {}", table_result.status().ToString()));
    }
    auto table = table_result.ValueOrDie();
...
void write_buffer(const FrameOrSeries& data, std::shared_ptr<arrow::Buffer>& buffer, const BinaryWriteOptions& options) {
    ...
    if (!resizable_buffer) {
        ...
        if (!new_buffer_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to allocate buffer: {}", new_buffer_result.status().ToString()));
        }
    ...
    if (!writer_result.ok()) {
        throw std::runtime_error(fmt::format("Failed to create IPC stream writer: {}", writer_result.status().ToString()));
    }
    ...
    while (true) {
        ...
        if (!batch_result.ok()) {
            throw std::runtime_error(fmt::format("Failed to read next batch: {}", batch_result.status().ToString()));
        }
        ...
        if (!status.ok()) {
            throw std::runtime_error(fmt::format("Failed to write record batch: {}", status.ToString()));
        }
    }
    ...
    if (!status.ok()) {
        throw std::runtime_error(fmt::format("Failed to close IPC stream writer: {}", status.ToString()));
    }
