...
namespace epoch_frame::calendar
{
    ...
    class Holiday
    {
      public:
        ...
        IndexPtr dates(arrow::TimestampScalar const& start_date,
                       arrow::TimestampScalar const& end_date) const;

        class Series dates_with_name(arrow::TimestampScalar const& start_date,
                                     arrow::TimestampScalar const& end_date) const;
    ...
    const HolidayData USLaborDay = {.name   = "Labor Day",
                                    .month  = std::chrono::September,
                                    .day    = 1d,
                                    .offset = {date_offset({.weekday = MO(1)})}};

    const HolidayData USColumbusDay = {.name   = "Columbus Day",
                                       .month  = std::chrono::October,
                                       .day    = 1d,
                                       .offset = {date_offset({.weekday = MO(2)})}};

    const HolidayData USThanksgivingDay = {.name   = "Thanksgiving Day",
                                           .month  = std::chrono::November,
                                           .day    = 1d,
                                           .offset = {date_offset({.weekday = TH(4)})}};

    const HolidayData USMartinLutherKingJr = {.name   = "Martin Luther King Jr. Day",
                                              .month  = std::chrono::January,
                                              .day    = 1d,
                                              .offset = {date_offset({.weekday = MO(3)})}};

    const HolidayData USPresidentsDay = {.name   = "Presidents Day",
                                         .month  = std::chrono::February,
                                         .day    = 1d,
                                         .offset = {date_offset({.weekday = MO(3)})}};

    const HolidayData GoodFriday = {.name   = "Good Friday",
                                    .month  = std::chrono::January,
                                    .day    = 1d,
                                    .offset = {epoch_frame::factory::offset::easter_offset(),
                                               epoch_frame::factory::offset::days(-2)}};

    const HolidayData EasterMonday = {.name   = "Easter Monday",
                                      .month  = std::chrono::January,
                                      .day    = 1d,
                                      .offset = {epoch_frame::factory::offset::easter_offset(),
