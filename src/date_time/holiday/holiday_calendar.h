...
namespace epoch_frame::calendar
{
    ...
    class HolidayCalendarRegistry
    {
      private:
        ...
        std::unordered_map<std::string, std::function<std::shared_ptr<AbstractHolidayCalendar>()>>
            calendar_factories;
    ...
    class AbstractHolidayCalendar
    {
      protected:
        ...
        DateTime                 start_date = DateTime(Date{1885y, std::chrono::January, 1d});
        DateTime                 end_date   = DateTime(Date{2200y, std::chrono::December, 31d});
      ...
      public:
        ...
        AbstractHolidayCalendar(const AbstractHolidayCalendarData& data,
                                DateTime start_date = {1885y, std::chrono::January, 1d},
                                DateTime end_date   = {2200y, std::chrono::December, 31d});
...
#define REGISTER_HOLIDAY_CALENDAR(CalendarRules)                                                   \
    ...
    const CalendarRules USFederalHolidayCalendar{
        HolidayData{.name       = "New Year's Day",
        ...
        USPresidentsDay,
        USMemorialDay,
        HolidayData{.name       = "Juneteenth National Independence Day",
                    .month      = std::chrono::June,
                    .day        = 19d,
                    .start_date = DateTime{Date{2021y, std::chrono::June, 18d}},
                    .observance = nearest_workday},
        HolidayData{.name       = "Independence Day",
                    .month      = std::chrono::July,
                    .day        = 4d,
                    .observance = nearest_workday},
        USLaborDay,
        USColumbusDay,
        HolidayData{.name       = "Veterans Day",
                    .month      = std::chrono::November,
                    .day        = 11d,
                    .observance = nearest_workday},
        USThanksgivingDay,
        HolidayData{.name       = "Christmas Day",
                    .month      = std::chrono::December,
                    .day        = 25d,
                    .observance = nearest_workday}};

    // Helper function to register a calendar
    inline void registerHolidayCalendar(const std::vector<HolidayData>& rules,
                                        std::string const&              name = "")
    {
        HolidayCalendarRegistry::getInstance().registerCalendar(
            AbstractHolidayCalendarData{rules, name});
    }
    ...
    inline AbstractHolidayCalendarPtr
    make_unnamed_calendar(const std::vector<HolidayData>& rules,
                          DateTime start_date = {1970y, std::chrono::January, 1d},
                          DateTime end_date   = {2200y, std::chrono::December, 31d})
    {
        return std::make_shared<AbstractHolidayCalendar>(
            AbstractHolidayCalendarData{rules}, std::move(start_date), std::move(end_date));
    }
} // namespace epoch_frame::calendar
