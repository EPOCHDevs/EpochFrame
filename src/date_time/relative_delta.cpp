...
namespace epoch_frame
{
    ...
    RelativeDelta::RelativeDelta(const RelativeDeltaOption& option)
    {
        if (option.dt1 && option.dt2)
        {
            ...
            auto months =
                (static_cast<int32_t>(dt1.date().year) - static_cast<int32_t>(dt2.date().year)) *
                    12 +
                (static_cast<int32_t>(static_cast<uint32_t>(dt1.date().month)) -
                 static_cast<int32_t>(static_cast<uint32_t>(dt2.date().month)));
            set_months(months);
    ...
    RelativeDelta RelativeDelta::normalized() const
    {
        ...
        return RelativeDelta(RelativeDeltaOption{.years        = m_years,
                                                 .months       = m_months,
    ...
    DateTime RelativeDelta::operator+(DateTime const& other) const
    {
        auto ret = other;
        auto year =
            static_cast<int32_t>(m_year.value_or(static_cast<int32_t>(ret.date().year))) + m_years;
        auto month =
            static_cast<int32_t>(m_month.value_or(static_cast<uint32_t>(ret.date().month)));

        if (m_months != 0)
        {
            AssertFromStream(1 <= std::abs(m_months) && std::abs(m_months) <= 12,
        ...
        auto day = std::min(static_cast<uint32_t>(std::chrono::year_month_day_last{
                                chrono_year{static_cast<int32_t>(year)},
                                chrono_month{static_cast<uint32_t>(month)} / std::chrono::last}
                                                      .day()),
                            m_day.value_or(static_cast<uint32_t>(ret.date().day)));
        ...
        // Add the hour, minute, second, and microsecond components from the original timestamp or
        // from the relative delta
        ret.replace(chrono_year{static_cast<int32_t>(year)})
        .replace(chrono_month{static_cast<uint32_t>(month)})
        .replace(chrono_day{day});
        if (m_hour.has_value())
        {
            ret.replace(chrono_hour{m_hour.value()});
        }
        if (m_minute.has_value())
        {
            ret.replace(chrono_minute{m_minute.value()});
        }
        if (m_second.has_value())
        {
            ret.replace(chrono_second{m_second.value()});
        }
        if (m_microsecond.has_value())
        {
            ret.replace(chrono_microsecond{m_microsecond.value()});
        }

        ret += TimeDelta{TimeDelta::Components{.days         = static_cast<double>(days),
        ...
        if (m_weekday.has_value())
        {
            ...
            ret += TimeDelta{TimeDelta::Components{.days = static_cast<double>(jumpdays)}};
        }
    ...
    {
        return RelativeDelta(RelativeDeltaOption{.years        = std::abs(m_years),
    ...
    {
        return m_years != 0 || m_months != 0 || m_days != 0 || m_hours != 0 || m_minutes != 0 ||
               m_seconds != 0 || m_microseconds != 0 || m_year.has_value() || m_month.has_value() ||
               m_day.has_value() || m_weekday.has_value() || m_hour.has_value() ||
               m_minute.has_value() || m_second.has_value() || m_microsecond.has_value();
    }

    RelativeDelta RelativeDelta::operator*(double const& other) const
    {
        return RelativeDelta(RelativeDeltaOption{.years        = m_years * other,
    ...
    bool RelativeDelta::operator==(const RelativeDelta& other) const
    {
        ...
        return m_years == other.m_years && m_months == other.m_months && m_days == other.m_days &&
               m_leapdays == other.m_leapdays && m_hours == other.m_hours &&
               m_minutes == other.m_minutes && m_seconds == other.m_seconds &&
               m_microseconds == other.m_microseconds && m_year == other.m_year &&
               m_month == other.m_month && m_day == other.m_day && m_weekday == other.m_weekday &&
    ...
    Date easter(int y)
    {
        ...
        auto h =
            static_cast<int64_t>(c - floor_div(c, 4) - floor_div(8 * c + 13, 25) + 19 * g + 15) %
            30;
        auto i = h - floor_div(h, 28) *
                         (1 - floor_div(h, 28) * floor_div(29, h + 1) * floor_div(21 - g, 11));
        ...
        return Date{chrono_year{static_cast<int32_t>(y)}, chrono_month{static_cast<uint32_t>(m)},
                    chrono_day{static_cast<uint32_t>(d)}};
    }
