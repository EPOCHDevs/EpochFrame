...
namespace epoch_frame::np
{
    ...
    void normalize_holiday_list(HolidayList& holidays, WeekMask& weekmask)
    {
        ...
        std::optional<DateTime> lastdate{std::nullopt};
        ...
        holidays.resize(trimcount);
    }

    auto find_earliest_holiday_on_or_after(std::optional<DateTime> const& date, auto holidays_begin, auto holidays_end)
    {
    ...
    auto find_earliest_holiday_after(std::optional<DateTime> const& date, auto holidays_begin, auto  holidays_end)
    {
        while (holidays_begin < holidays_end) {
            auto trial = holidays_begin + (holidays_end - holidays_begin) / 2;
            if (date < *trial) {
    ...
    DateTime apply_business_day_offset(DateTime date, int64_t offset, BusDayOffsetRoll roll, WeekMask const& weekmask, uint8_t busdays_in_weekmask, auto holidays_begin, auto holidays_end)
    {
        ...
        if (offset > 0) {
            ...
            auto holidays_temp = find_earliest_holiday_on_or_after(date, holidays_begin, holidays_end);
    ...
    int64_t apply_business_day_count(DateTime date_begin, DateTime date_end, WeekMask const& weekmask, uint8_t busdays_in_weekmask, auto holidays_begin, auto holidays_end)
    {
       ...
       whole_weeks = (date_end - date_begin).days() / 7;
    ...
    {
        validate_busdays_in_week(static_cast<int8_t>(busdays_in_weekmask));
    ...
    BusinessDayCalendar::BusinessDayCalendar(WeekMask const& weekmask, HolidayList const& holidays) : m_weekmask(weekmask), m_holidays(holidays) {
        // Count the number of business days manually
        m_busdays_in_weekmask = std::ranges::count(weekmask, true);
        AssertFromFormat(m_busdays_in_weekmask > 0, "Cannot construct a busdaycal with a weekmask of all false values");

        normalize_holiday_list(m_holidays, m_weekmask);
    }
    ...
    int64_t BusinessDayCalendar::count(DateTime const& dates_begin, DateTime const& dates_end) {
        return apply_business_day_count(dates_begin, dates_end, m_weekmask, m_busdays_in_weekmask, m_holidays.begin(), m_holidays.end());
    }
    ...
    WeekMask to_weekmask(WeekSet const& weekmask) {
        WeekMask result{false, false, false, false, false, false, false};
