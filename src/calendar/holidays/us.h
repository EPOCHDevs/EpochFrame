...
/*
 * These holiday definitions are based on the pandas_market_calendars/holidays/us.py implementation
 */

// Holiday observance functions
constexpr auto following_tuesday_every_four_years_observance = [](const DateTime& dt) -> DateTime {
    auto offset = date_offset(RelativeDeltaOption{.years = static_cast<double>((4 - (static_cast<int>(dt.date.year) % 4)) % 4), .weekday = TU(1)});
...
const std::vector<DateTime> WeatherSnowClosing = {
    "1969-02-10"__date
};
...
const std::vector<DateTime> HurricaneGloriaClosings = {
    "1985-09-27"__date
};
