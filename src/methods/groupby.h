...
namespace epoch_frame {
    ...
    class Grouper {
    public:
        explicit Grouper(arrow::TablePtr table, std::optional<TimeGrouperOptions> options = std::nullopt);
    ...
    public:
        KeyGrouper(arrow::TablePtr table, std::vector<std::string> const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
    ...
    public:
        ArrayGrouper(arrow::TablePtr table, arrow::ChunkedArrayVector const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
    ...
    public:
        ...
        [[nodiscard]] OutputType agg(std::string const& agg_name,
            const std::shared_ptr<arrow::compute::FunctionOptions>& option = nullptr) const;

        [[nodiscard]] std::unordered_map<std::string, OutputType> agg(
            std::vector<std::string> const& agg_names,
            const std::vector<std::shared_ptr<arrow::compute::FunctionOptions>>& options) const;
        ...
        OutputType count(arrow::compute::CountOptions::CountMode mode = arrow::compute::CountOptions::CountMode::ONLY_VALID) const { 
        ...
        OutputType count_distinct(arrow::compute::CountOptions::CountMode mode = arrow::compute::CountOptions::CountMode::ONLY_VALID) const { 
        ...
        OutputType quantile(double q, arrow::compute::QuantileOptions::Interpolation interpolation = arrow::compute::QuantileOptions::Interpolation::LINEAR) const { 
            return agg("quantile", std::make_shared<arrow::compute::QuantileOptions>(q, interpolation)); 
        }  
        ...
        OutputType variance(int ddof = 0, bool skip_nulls = true, uint64_t min_count = 0) const { 
            return agg("variance", std::make_shared<arrow::compute::VarianceOptions>(ddof, skip_nulls, min_count)); 
        }  
    ...
    namespace factory::group_by {
        ...
        template<typename OutputType>
        GroupByAgg<OutputType> make_agg_by_key(arrow::TablePtr table, std::vector<std::string> const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
        
        template<typename OutputType>
        GroupByAgg<OutputType> make_agg_by_array(arrow::TablePtr table, arrow::ChunkedArrayVector const &by, std::optional<TimeGrouperOptions> options = std::nullopt);

        template<typename OutputType>
        GroupByAgg<OutputType> make_agg_by_index(DataFrame const& table, const TimeGrouperOptions& options);

        // For apply operations only
        GroupByApply make_apply_by_key(DataFrame const& table, std::vector<std::string> const &by, bool groupKeys, std::optional<TimeGrouperOptions> options = std::nullopt);
        GroupByApply make_apply_by_array(DataFrame const&  table, arrow::ChunkedArrayVector const &by, bool groupKeys, std::optional<TimeGrouperOptions> options = std::nullopt);
        GroupByApply make_apply_by_index(DataFrame const& table, bool groupKeys, const TimeGrouperOptions& options);

        extern template GroupByAgg<DataFrame> make_agg_by_key<DataFrame>(arrow::TablePtr table, std::vector<std::string> const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
        extern template GroupByAgg<DataFrame> make_agg_by_array<DataFrame>(arrow::TablePtr table, arrow::ChunkedArrayVector const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
        extern template GroupByAgg<DataFrame> make_agg_by_index<DataFrame>(DataFrame const& table, const TimeGrouperOptions& options);

        extern template GroupByAgg<Series> make_agg_by_key<Series>(arrow::TablePtr table, std::vector<std::string> const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
        extern template GroupByAgg<Series> make_agg_by_array<Series>(arrow::TablePtr table, arrow::ChunkedArrayVector const &by, std::optional<TimeGrouperOptions> options = std::nullopt);
        extern template GroupByAgg<Series> make_agg_by_index<Series>(DataFrame const& table, const TimeGrouperOptions& options);
    }
}
