...
namespace epoch_frame
{
    ...
    SeriesOrScalar Aggregator::agg(AxisType axis, std::string const& agg, bool skip_null,
                                   arrow::compute::FunctionOptions const& options) const
    {
        if (m_data.second.is_chunked_array())
        {
            ...
            return SeriesOrScalar{Scalar{
                arrow_utils::call_unary_agg_compute(m_data.second.chunked_array(), agg, options)}};
        }
                ...
                [&]
                {
                    parallel_for(
                        ...
                        [&](const tbb::blocked_range<int64_t>& r)
                        {
                            ...
                            {
                                auto field      = fields[i];
                                auto agg_result = AssertContiguousArrayResultIsOk(
                                    arrow::MakeArrayFromScalar(*arrow_utils::call_unary_agg_compute(
                                                                   table->column(i), agg, options),
                                                               1));
                                auto index =
                                    AssertContiguousArrayResultIsOk(arrow::MakeArrayFromScalar(
                                        *arrow::MakeScalar(field->name()), 1));

                                const auto schema =
                                    arrow::schema({arrow::field("index", arrow::utf8()),
                                                   arrow::field("value", field->type())});
                                tables[i] = AssertTableResultIsOk(arrow::Table::Make(
                                    schema, arrow::ArrayVector{index, agg_result}));
                            }
                        },
                        tbb::simple_partitioner());
                });

            auto concat_table = AssertTableResultIsOk(arrow::ConcatenateTables(
                tables,
                arrow::ConcatenateTablesOptions{true, arrow::Field::MergeOptions::Permissive()}));
            auto index =
                factory::array::make_contiguous_array(concat_table->GetColumnByName("index"));
            auto value = concat_table->GetColumnByName("value");
            AssertFromStream(index != nullptr && value != nullptr,
                             "IIndex or value column not found");
        ...
        if (agg == "min" || agg == "max")
        {
            ...
            auto result = CallFunction(::std::format("{}_element_wise", agg), inputs,
        ...
        parallel_for(tbb::blocked_range<int64_t>(0, table->num_rows()),
                     [&](const tbb::blocked_range<int64_t>& r)
                     {
                         for (int64_t i = r.begin(); i != r.end(); ++i)
                         {
                             ...
                             scalarVector[i] =
                                 arrow_utils::call_unary_agg_compute(array, agg, options);
                         }
                     });
    ...
    FrameOrSeries Aggregator::mode(AxisType axis, bool skip_null, int64_t n) const
    {
        ...
        if (m_data.second.is_chunked_array())
        {
            auto arr = AssertResultIsOk(Mode(m_data.second.chunked_array(), options))
        ...
        {
            auto table = m_data.second.table();
            return FrameOrSeries{DataFrame{arrow_utils::apply_function_to_table(
                table,
                [&](arrow::Datum const& datum, std::string const& column)
                {
