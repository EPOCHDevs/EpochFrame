...
namespace epoch_frame {
    ...
    public:
        ...
        SeriesOrScalar agg(AxisType axis, std::string const& agg_name, bool skip_null, int64_t min_count=0) const {
            return agg(axis, agg_name, skip_null, arrow::compute::ScalarAggregateOptions(skip_null, min_count));
        }

        SeriesOrScalar all(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
            return agg(axis, "all", skip_null, min_count);
        }

        SeriesOrScalar any(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
            return agg(axis, "any", skip_null, min_count);
        }

        SeriesOrScalar approximate_median(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
        ...
        SeriesOrScalar first(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
            return agg(axis, "first", skip_null, min_count);
        }

        SeriesOrScalar last(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
        ...
        SeriesOrScalar max(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
            return agg(axis, "max", skip_null, min_count);
        }

        SeriesOrScalar min(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
            return agg(axis, "min", skip_null, min_count);
        }

        SeriesOrScalar mean(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
        ...
        SeriesOrScalar sum(AxisType axis=AxisType::Row, bool skip_null=true, int64_t min_count=0) const {
