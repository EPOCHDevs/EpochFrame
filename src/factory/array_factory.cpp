...
namespace epoch_frame::factory::array
{
                                              ...
                                              std::shared_ptr<arrow::DataType> const& type)
    {
        ...
        return AssertArrayResultIsOk(
            arrow::ChunkedArray::Make(std::vector<arrow::ArrayPtr>{result}));
    }
                                          ...
                                          std::shared_ptr<arrow::DataType> const& type)
    {
        ...
        std::ranges::transform(set_array, scalars.begin(),
                               [](auto const& scalar) { return scalar.value(); });
                                 ...
                                 const std::vector<std::shared_ptr<arrow::Field>>& fields)
    {
        ...
        {
            return arrow::Status::Invalid("Number of columns does not match number of fields");
        }
        if (columns.empty())
        {
    ...
    arrow::ArrayPtr make_timestamp_array(const std::vector<arrow::TimestampScalar>& set_array,
                                         arrow::TimeUnit::type unit, std::string const& timezone)
    {
        arrow::TimestampBuilder builder(arrow::timestamp(unit, timezone),
    ...
    arrow::ChunkedArrayPtr join_chunked_arrays(arrow::ArrayPtr const&        x,
                                               const arrow::ChunkedArrayPtr& chunked_array,
                                               bool                          join_right)
    {
        arrow::ArrayVector arrays;
        if (join_right)
        {
            arrays.push_back(x);
            arrays.insert(arrays.end(), chunked_array->chunks().begin(),
     ...
     */
    arrow::ChunkedArrayPtr make_random_normal_array(int64_t length, int64_t seed, double mean,
                                                    double stddev)
    {
        ...
        return make_array<double>(random_data);
    }
    ...
    make_random_normal_array_for_index(std::shared_ptr<arrow::ChunkedArray> date_index,
                                       int64_t seed, double mean, double stddev)
    {
        return make_random_normal_array(date_index->length(), seed, mean, stddev);
    }

    arrow::ChunkedArrayPtr make_random_normal_array_for_index(IndexPtr const& index, int64_t seed,
                                                              double mean, double stddev)
    {
        return make_random_normal_array(index->size(), seed, mean, stddev);
    }
} // namespace epoch_frame::factory::array
