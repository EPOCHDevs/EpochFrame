...
// Your includes:
#include "../index_factory.h"  // Contains ObjectIndex and the make_object_index(...) declarations
...
TEST_CASE("make_object_index from std::vector<arrow::ScalarPtr>", "[object_index]")
{
    ...
    SECTION("Some null scalars") {
        ...
        // Check values
        REQUIRE(str_arr->IsValid(0));
        REQUIRE(str_arr->GetString(0) == "non-null");

        // The middle element is null
        REQUIRE_FALSE(str_arr->IsValid(1));

        // The last is valid
        REQUIRE(str_arr->IsValid(2));
        REQUIRE(str_arr->GetString(2) == "another");
    }
    ...
    SECTION("All null scalars") {
        ...
        REQUIRE_FALSE(str_arr->IsValid(0));
        REQUIRE_FALSE(str_arr->IsValid(1));
    }
