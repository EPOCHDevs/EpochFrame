...
{
    DataFrame make_empty_dataframe(arrow::DataTypePtr const& indexType,
                                   arrow::SchemaPtr const&   schema)
    {
        return DataFrame(factory::index::make_index(
                             arrow::MakeEmptyArray(indexType).MoveValueUnsafe(), std::nullopt, ""),
                         factory::table::make_empty_table(schema));
    }
                             ...
                             arrow::DataTypePtr const&       type)
    {
        ...
        return make_dataframe(index, arrow::Table::Make(arrow::schema(fields), columns));
    }

    DataFrame make_dataframe(IndexPtr const& index, std::vector<arrow::ChunkedArrayPtr> const& data,
                             std::vector<std::string> const& columnNames)
    {
        AssertFromStream(data.size() == columnNames.size(),
                         "Data and column names must have the same size");
        arrow::FieldVector fields;
                             ...
                             arrow::FieldVector const& fields)
    {
        ...
        return make_dataframe(index, arrow::Table::Make(schema(fields), columns));
    }
} // namespace epoch_frame
