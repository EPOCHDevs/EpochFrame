...
namespace epoch_frame
{
     ...
     */
    template <bool IsMonotonic> class ArrowIndex : public IIndex
    {
      public:
        //--------------------------------------------------------------------------
        // Constructor
        ...
        IndexPtr drop(Array const& labels) const final
        {
        ...
        std::vector<int64_t> get_loc(Scalar const& label) const final;
        std::vector<int64_t> get_loc(IndexPtr const& other) const final;

        ResolvedIntegerSliceBound slice_locs(Scalar const& start,
                                             Scalar const& end = {}) const final;
        ...
        IndexPtr filter(Array const& bool_filter, bool drop_null) const final
        {
            return Make(AssertContiguousArrayResultIsOk(Filter(
                arrow::Datum{m_array.value()}, arrow::Datum{bool_filter.value()},
                arrow::compute::FilterOptions{drop_null
                                                  ? arrow::compute::FilterOptions::DROP
                                                  : arrow::compute::FilterOptions::EMIT_NULL})));
        }

        IndexPtr take(Array const& indices, bool bounds_check) const final
        {
            return Make(AssertContiguousArrayResultIsOk(
                arrow::compute::Take(arrow::Datum{m_array.value()}, arrow::Datum{indices.value()},
                                     arrow::compute::TakeOptions{bounds_check})));
        }

        IndexPtr
        where(Array const&                                         conditions,
              arrow::compute::FilterOptions::NullSelectionBehavior null_selection) const final
        {
            return Make(AssertContiguousArrayResultIsOk(arrow::compute::Filter(
                arrow::Datum{m_array.value()}, arrow::Datum{conditions.value()},
        ...
        ScalarMapping<std::vector<int64_t>> indexer() const
        {
            return get_indexer();
        }
